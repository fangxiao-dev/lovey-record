...
export async function getOrCreateModuleInstance(user: { id: string; openid: string }) {
  const existing = await prisma.moduleInstance.findFirst({
    where: {
      ownerUserId: user.id,
      moduleType: 'menstrual',
  ...
  if (existing) {
    return { moduleInstanceId: existing.id, profileId: existing.profileId };
  }
  ...
  return { moduleInstanceId: moduleInstance.id, profileId: profile.id };
}
