...
- `经期` chip: toggles period status for this date. Independent of attribute recording.
...
- Tap `经期` when inactive: marks the date as a period day, chip transitions to active accent state.
- Tap `经期` when active: un-marks the date, chip returns to inactive state.
- Toggling period status does not affect recorded attributes. They are independent data axes.
